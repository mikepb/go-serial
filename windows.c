...
static char *get_external_hub_name(HANDLE hub, ULONG connection_index)
{
	...
	if (size <= sizeof(ext_hub_name) || !(ext_hub_name_wc = malloc(size)))
		return NULL;
								   ...
								   UCHAR descriptor_index)
{
	...
	desc_req->SetupPacket.wValue = (USB_STRING_DESCRIPTOR_TYPE << 8) | descriptor_index;
	...
	if (!DeviceIoControl(hub_device,
		...
		size < 2 || desc->bDescriptorType != USB_STRING_DESCRIPTOR_TYPE || desc->bLength != size - sizeof(*desc_req) || desc->bLength % 2)
		return NULL;
...
static void get_usb_details(struct sp_port *port, DEVINST dev_inst_match)
{
	...
	for (i = 0; SetupDiEnumDeviceInfo(device_info, i, &device_info_data); i++)
	{
		...
		while (CM_Get_Parent(&dev_inst, dev_inst, 0) == CR_SUCCESS && dev_inst != device_info_data.DevInst)
		{
		}
